...
| 버그제보 | 에스컬레이션 | high |
| 기능요청 | 에스컬레이션 | medium |
| 불만 | 우선순위로 분기 | low→답변 / high→에스컬레이션 |
| 칭찬 | 답변 경로 | low |
| 기타 | 에스컬레이션 | medium |
...
3. 분류 유형별 파일 가중치: 사용법문의·칭찬 → `01~03` 우선 / 불만·버그제보 → `04~05` 우선
4. 상위 2~3개 청크 + 출처 메타데이터(`파일명#섹션제목`) 반환
...
- 다중 턴 대화 (1 VOC = 1 처리, 세션은 분류용 식별자로만 사용)
- VOC 대량 일괄 처리/스케줄러
