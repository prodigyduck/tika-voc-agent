...
- [ ] **Step 5: dev DB 스키마 노트** — `create_all`은 기존 테이블에 컬럼을 추가하지 않는다. 로컬 `doit_voc_agent.db`가 있으면 `rm doit_voc_agent.db`로 재생성 (gitignored, 개발 데이터만 손실). 커밋 메시지 본문에도 명시.
...
2. `rm -f doit_voc_agent.db &&` 백엔드 기동 후 대시보드 수동 확인(배지·카드·필터)
